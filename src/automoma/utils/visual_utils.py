...
def visualize_voxel_grid_with_cuboid(
    ...
    # color_inside=[255, 0, 0, 255],   # Red, opaque
    color_inside=[0, 0, 255, 255],  
    cuboid_outline_color=[0, 255, 0, 255],  # Green
    show_cuboid_outline=True,
    show_mesh=True
):
    """
    Visualize the voxel grid using trimesh, highlighting occupied points inside/outside a cuboid.
    Only occupied voxels are shown for clarity.
    Args:
        cuboid (Cuboid): The cuboid region to highlight.
        voxel_grid (VoxelGrid): The voxel grid to visualize.
        threshold (float): Threshold to determine occupied voxels.
        mesh_obstacle (Optional[Mesh]): Mesh obstacle to show.
        color_outside (list): RGBA for occupied voxels outside cuboid.
        color_inside (list): RGBA for occupied voxels inside cuboid.
        cuboid_outline_color (list): RGBA for cuboid outline.
        show_cuboid_outline (bool): Whether to show cuboid outline.
        show_mesh (bool): Whether to show mesh_obstacle.
    """
    import trimesh
    import numpy as np
    ...
    # Cuboid outline (as green wireframe box)
    ...
    if show_mesh and mesh_obstacle is not None:
        try:
            mesh_trimesh = mesh_obstacle.get_trimesh_mesh()
            mesh_transform = np.eye(4)
            mesh_transform[:3, 3] = mesh_obstacle.pose[:3]
            if len(mesh_obstacle.pose) == 7:
                from scipy.spatial.transform import Rotation as R
                mesh_quat = mesh_obstacle.pose[3:]
                mesh_rot = R.from_quat([mesh_quat[1], mesh_quat[2], mesh_quat[3], mesh_quat[0]]).as_matrix()
    ...
    # Optionally, set dark background if your viewer supports it
    try:
