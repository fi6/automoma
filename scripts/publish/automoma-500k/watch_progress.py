...
def launch_codex_debug(
    *,
    ...
    pid_file = log_dir / "codex_debug.pid"
    existing_pid = read_pid(pid_file)
    if existing_pid is not None and pid_alive(existing_pid):
        return [f"codex debug already running pid={existing_pid}"]

    now = datetime.now().astimezone().strftime("%Y%m%d_%H%M%S")
    prompt_file = log_dir / f"codex_debug_prompt_{now}.md"
    log_file = log_dir / f"codex_debug_{now}.log"
    dead_summary = "\n".join(
        f"- worker {worker['worker']}: pid={worker['pid']} cmd={worker['cmd_file']} log={worker['log_file']}"
        for worker in dead_workers
    )
    progress_summary = "\n".join(
        f"- {obj}: {item['success']}/{target_per_object}, remaining {max(0, target_per_object - item['success'])}"
        for obj, item in sorted(progress.items())
    )
    total_success = sum(item["success"] for item in progress.values())
    prompt = f"""You are an autonomous Codex debug agent running because AutoMoMa-500k planning workers died before reaching the target.

Repository: {REPO_ROOT}
Current log directory: {log_dir}
Target: {target_total} total successful trajectories, {target_per_object} per object.
Current total: {total_success}/{target_total}.
...
Instructions:
- Diagnose the dead worker logs and recent canonical trajectory counts.
- Fix first-party code/scripts if needed. Do not edit third_party without explicit confirmation.
- Preserve existing logs and append restart markers.
- Restart only the dead/needed worker(s) using their gpu*.cmd.sh files after fixing the cause.
- Update logs/automoma-500k-progress.md with what happened, what you changed, and the restarted pid(s).
- Keep working until the failed planning workers are running again or the target is already complete.
"""
    ...
    with prompt_file.open("r", encoding="utf-8") as stdin, log_file.open("a", encoding="utf-8") as stdout:
        proc = subprocess.Popen(
            cmd,
            cwd=REPO_ROOT,
            stdin=stdin,
    ...
    actions.append(f"started codex --yolo debug pid={proc.pid} log={log_file} prompt={prompt_file}")
    return actions
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--codex-debug-dead", action="store_true")
...
def main() -> int:
    ...
    while True:
        ...
        debug_actions = []
        if args.codex_debug_dead and dead_workers and not all_done:
            debug_actions = launch_codex_debug(
            ...
            actions.extend(debug_actions)
        if args.restart_dead and not all_done and dead_workers and not debug_actions:
