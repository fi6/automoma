...
This document covers the first-party release tools for preparing the legacy AutoMoMa 30-scene HDF5 dataset without modifying the original files.
...
```bash
python tools/release/automoma-30k/organize_hdf5.py --force
```
...
```bash
python tools/release/automoma-30k/organize_hdf5.py --force --link-mode manifest
```
