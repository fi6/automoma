...
#
# Example:
#   bash tools/release/automoma-500k/run.sh --GPUS 2
#   bash tools/release/automoma-500k/run.sh --GPUS 2 -- planner.traj.batch_size=10
...
while true; do
  ...
  if ((live == 0)); then
    ...
    "${PYTHON_BIN}" "${SCRIPT_DIR}/trajectory_statistics.py" \
      --config "${CONFIG}" \
      --root "${OUTPUT_ROOT}" \
