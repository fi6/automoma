...
CMD="cd '$REPO_ROOT' && source /home/xinhai/miniconda3/etc/profile.d/conda.sh && conda activate automoma && '$PYTHON_BIN' scripts/run_grasp_filter_metrics.py --output-dir '$OUTPUT_DIR' --gpu '$GPU' --scenes-per-object '$SCENES_PER_OBJECT' --keep-going $EXTRA_ARGS 2>&1 | tee -a '$OUTPUT_DIR/run.log'"
